...
2. Annotate each decision with UI picker attributes:
   ...
   - Apply the Tailwind CSS `contents` class to wrapper and option nodes so wrappers do not affect layout. If the project does not use Tailwind, use `style="display: contents"` on wrapper and option nodes instead of the `contents` class.
   - Tailwind v3 trap: preflight's `[hidden] { display: none }` and the `.contents` utility have equal specificity, and `.contents` wins the cascade, so `hidden` options can still render. After annotating, verify hidden options actually do not render; if they do, either add `[hidden]{display:none!important}` to global CSS for the comparison round (remove it at final cleanup) or set inline `style="display:none"` on the non-visible options. Tailwind v4 is safe (its `[hidden]` rule uses `!important`).
...
6. Ask for selection in the agent using the `AskUserQuestion` tool (or the agent's equivalent structured-question tool):
...
- Confirm hidden options are actually hidden (see the Tailwind v3 trap in the Workflow).
...
- Use the Tailwind CSS `contents` class on wrapper and option nodes; if the project does not use Tailwind, use `style="display: contents"` instead.
...
- The picker script is comparison-only tooling. If the user asks to commit or deploy while the toolbar script is still injected, flag it and offer to remove it first.
