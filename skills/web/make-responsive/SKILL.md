...
Rules are written for Tailwind CSS v4. On non-Tailwind or Tailwind v3 projects, apply the same patterns via the project's styling system (media/container queries, `min-width: 0`, `flex-shrink: 0`, `overflow-x: auto`) instead of copying class names verbatim.
...
- Use `min-h-dvh` (or `min-h-svh` for fixed app shells); never use `min-h-screen`
...
- If a text input's font size is smaller than `16px`, add `max-sm:text-base/6` (keep the element's existing line-height) to bump it to `16px` on mobile
...
- Confirm the UI works at 375px (below `sm`), 768px (`md`), and 1280px (`lg`+) — via browser devtools responsive mode or the project's screenshot tooling — matching the project's configured breakpoints if customized.
