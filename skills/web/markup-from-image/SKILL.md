...
1. Inspect the source image and the user's prompt to determine intended output format, target file, insertion location, and source scope (`full page`, `page section`, `component`, or `embedded media`).
...
- Reserve `<h1>` for full-page scope (page title or hero); start section- and component-scope markup at `<h2>` and keep heading levels sequential, or match the surrounding heading hierarchy of the target file when inserting.
...
- Pair form controls with visible `label` elements when the image shows labels, associating each label explicitly by either wrapping the control inside the `<label>` or pairing `for` (`htmlFor` in JSX) with a matching `id` on the control; use `aria-label` when a control has no visible label; use `fieldset` and `legend` for grouped controls.
- Represent icons as a 20px by 20px `<svg>` with `aria-hidden="true"` and only a comment naming the source icon's inferred meaning; never use `<img>` for icon placeholders.
- Give icon-only `<button>` and `<a>` elements an `aria-label` naming the inferred action or destination, since their `<svg>` icon is decorative and carries no accessible name.
- Treat app screenshots, UI mockups, interface previews, dashboards, charts, maps, code editors, device screens, browser windows, and product screenshots as media; represent them as a placeholder `<img src="#" alt="<description of what the image shows>" />` instead of recreating their internal UI as markup.
- Represent meaningful images, logos, avatars, screenshots, and thumbnails as a placeholder `<img src="#" alt="..." />` when needed, using `src="#"` (or an empty-string `src`) with no external placeholder services and no data URIs; use empty `alt` text only for decorative or unidentified imagery.
...
- Confirm lists, tables, forms, navigation, buttons, links, headings, landmarks, and media use appropriate native semantics, including accessible names for form controls and for icon-only buttons and links.
- Confirm every `<section>` has a logical kebab-case `id` based on its content or purpose.
- Confirm the chosen scope matches the prompt, requested file/component name, insertion target, and visible image context; isolated sections/components must not be wrapped in page-level `<main>`, and `<h1>` appears only in full-page scope while section/component scope starts at `<h2>` with sequential levels.
...
- Confirm icon placeholders use 20px by 20px `<svg>` elements marked `aria-hidden="true"` with only an inferred-meaning comment, no `<title>`, and no `<img>` elements.
- Confirm embedded app/interface screenshots and other media are represented as placeholder `<img src="#" alt="...">` elements (no external placeholder services, no data URIs), not recreated as nested controls, tables, charts, browser chrome, or device UI markup.
