...
- When adding shadows to buttons, follow the shadow rules in [Shadows](./shadows.md) — never pair `shadow-*` with solid gray borders; use `ring-1 ring-black/5` or `ring-1 ring-black/10` instead
...
- Application UIs (dashboards, settings, admin) — `text-sm` with compact padding, never `text-base`; total rendered button height (including outer wrapper/ring) must stay within 28–38px; if the button uses a wrapper element for its border/ring (e.g. `p-px`), include its size in the height budget (a `p-px` wrapper adds 2px total)
