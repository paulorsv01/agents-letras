...
- Replace `{brand}` with the appropriate brand color token, including the shade (e.g. `indigo-600`)
...
- The styles are all applied in CSS based on the input state
- **Never use JavaScript to toggle classes based on input state** — use CSS states and variants exclusively
- Replace `{brand}` with the appropriate brand color family, without a shade (e.g. `indigo`); the snippet appends the shade, as in `has-checked:bg-{brand}-600`
- Replace `{gray}` with the appropriate gray color family, without a shade (e.g. `gray`); the snippet appends the shade, as in `bg-{gray}-200`
- Every class is required, do not omit any
