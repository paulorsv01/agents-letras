...
if [ -z "${APP_ID:-}" ]; then
    echo "Error: couldn't resolve applicationId. Is this an Android project root?" >&2
    exit 1
fi
...
if [ -z "${DEVICE:-}" ]; then
    case "$TARGET_KIND" in
        emulator)
            echo "No emulator connected. Run: make emulator" >&2
            ;;
        device)
            echo "No physical device connected." >&2
            ;;
        *)
            echo "No device/emulator connected. Run: make emulator" >&2
            ;;
    esac
    exit 1
fi
...
ACTIVITY=$(adb -s "$DEVICE" shell cmd package resolve-activity --brief "$APP_ID" | tail -n 1 | tr -d '\r')
if [ -z "${ACTIVITY:-}" ]; then
