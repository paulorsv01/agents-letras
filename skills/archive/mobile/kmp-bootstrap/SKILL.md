---
name: kmp-bootstrap
description: "Scaffold, build, test, run, and release Kotlin Multiplatform Android+iOS apps using the AGP 9 split-module layout."
---
...
- `assets/templates/scripts/ios_run.sh`: Build, install, and launch on simulator or physical iPhone. Flags: `--target any|simulator|device`, `--derived-data <path>`, optional UDID/name. Resolves `.app` via `xcodebuild -showBuildSettings` (TARGET_BUILD_DIR + WRAPPER_NAME) instead of guessing with `find`. Device path uses `xcrun devicectl`.
...
- **Activity resolution**: `cmd package resolve-activity --brief` can prefix the component line with whitespace, which `am start -n` rejects as an invalid component name. Always extract via `awk '/\// { last=$NF }`, never `tail -n 1` or `$0`.
- **iOS `.app` resolution**: parse `TARGET_BUILD_DIR` + `WRAPPER_NAME`/`FULL_PRODUCT_NAME` from `xcodebuild -showBuildSettings` instead of `find ... -name '*.app'`. The find approach picks the wrong artifact on multi-product schemes and on schemes that produce extensions.
- **iOS device path**: physical iPhone uses `xcrun devicectl` (`device install app`, `device process launch`). Discover device id from `xcodebuild -showdestinations`, filtering out `placeholder`/`DVTiPhonePlaceholder`. Simulator path stays on `xcrun simctl install` / `simctl launch`.
