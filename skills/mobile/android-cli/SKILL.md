...
- start it directly: `adb shell am start -W -n <package>/<activity>`

This is more reliable than `monkey` when you need deterministic launch behavior.

## Managing emulators
Manage Android Virtual Devices (AVDs) using the `android emulator` command

For automation, treat `android emulator` as discovery and simple control, not the final source of truth for boot readiness:
- `android emulator list` is fine for quick discovery, but the SDK `emulator` binary is the better automation entrypoint.
- Some environments report "ready" too early or behave poorly when launched only through the wrapper.
- For durable scripting, prefer the SDK `emulator` binary when available, then wait for all of:
  - `getprop sys.boot_completed = 1`
  - `getprop dev.bootcomplete = 1`
  - `getprop init.svc.bootanim = stopped`
  - `adb shell pm path android`
  - `adb shell pm list packages`
- If a warm boot comes up unhealthy, retry with a cold boot (`-no-snapshot-load`) before jumping to heavier recovery.
- If the emulator still cannot answer `pm` queries or install APKs, wipe/recreate the AVD instead of pretending the device is usable.
