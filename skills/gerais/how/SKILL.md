...
Spawn all explorers in a single message. Each one must stay read-only — Read, Grep, Glob only, no mutations.
...
Spawn a single subagent, read-only, that explores and explains in one pass.
...
Once all explorers have returned, spawn a single subagent, read-only, to synthesize their findings into one coherent explanation.
...
After the explanation is complete, spawn architectural critics in parallel. Each critic is read-only and independent, so they form judgments without anchoring on each other. Three critics is a good default.
